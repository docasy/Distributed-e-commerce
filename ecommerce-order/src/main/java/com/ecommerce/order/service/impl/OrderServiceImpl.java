...
public class OrderServiceImpl implements OrderService {
    ...
    public Order createOrder(Long userId, CreateOrderDTO createOrderDTO) {
        ...
        // 3. 检查库存
        Map<String, Object> stockResult = productFeignClient.checkStock(
                createOrderDTO.getProductId(),
                createOrderDTO.getQuantity()
        );
        if (stockResult == null || !Boolean.TRUE.equals(stockResult.get("data"))) {
            throw new BusinessException(ResultCode.PRODUCT_STOCK_NOT_ENOUGH);
        }

        // 4. 扣减库存
        Map<String, Object> deductResult = productFeignClient.deductStock(
                createOrderDTO.getProductId(),
                createOrderDTO.getQuantity()
        );
        if (deductResult == null || !Boolean.TRUE.equals(deductResult.get("data"))) {
            throw new BusinessException(ResultCode.PRODUCT_STOCK_NOT_ENOUGH);
        }

        // 5. 查询收货地址（通过Feign调用用户服务）
        String addressStr = "";
        String receiverName = "";
        String receiverPhone = "";
        try {
            Map<String, Object> addrResult = userFeignClient.getAddressById(createOrderDTO.getAddressId());
            if (addrResult != null && addrResult.get("data") != null) {
                @SuppressWarnings("unchecked")
                Map<String, Object> addr = (Map<String, Object>) addrResult.get("data");
                receiverName = addr.get("receiverName") != null ? addr.get("receiverName").toString() : "";
                receiverPhone = addr.get("receiverPhone") != null ? addr.get("receiverPhone").toString() : "";
                addressStr = (addr.get("province") != null ? addr.get("province").toString() : "")
                    + (addr.get("city") != null ? addr.get("city").toString() : "")
                    + (addr.get("district") != null ? addr.get("district").toString() : "")
                    + " " + (addr.get("detail") != null ? addr.get("detail").toString() : "");
            }
        } catch (Exception e) {
            log.error("查询地址失败：{}", e.getMessage());
        }
    ...
    public boolean cancelOrder(String orderNo) {
        ...
        if (order.getStatus() != 0) {
            // 如果已经是已取消状态（上次 Feign 失败导致的），只补调恢复库存
            if (order.getStatus() == 2) {
                productFeignClient.addStock(order.getProductId(), order.getQuantity(), order.getOrderNo());
                log.info("订单取消补偿：恢复库存成功，{}", orderNo);
                return true;
            }

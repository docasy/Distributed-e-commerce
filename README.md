# 分布式电商平台 

<p align="center">
  <img src="https://img.shields.io/badge/Spring%20Boot-2.7.18-brightgreen.svg" alt="Spring Boot"/>
  <img src="https://img.shields.io/badge/Spring%20Cloud-2021.0.8-blue.svg" alt="Spring Cloud"/>
  <img src="https://img.shields.io/badge/Java-11-orange.svg" alt="Java"/>
  <img src="https://img.shields.io/badge/MySQL-8.0-blue.svg" alt="MySQL"/>
  <img src="https://img.shields.io/badge/Redis-7.0-red.svg" alt="Redis"/>
</p>

## 📖 项目简介

这是一个基于**微服务架构**的分布式电商平台后端项目。项目涵盖了分布式系统中的核心技术点，包括：

-  **微服务架构**：用户、商品、订单服务独立部署
-  **分布式锁**：Redis实现防止商品超卖
-  **接口幂等性**：Token机制防止订单重复提交
-  **消息队列**：RabbitMQ实现订单超时自动取消
-  **服务调用**：OpenFeign实现服务间通信
-  **网关鉴权**：Spring Cloud Gateway统一JWT认证
-  **限流降级**：Gateway集成Redis限流
-  **缓存策略**：Redis缓存热点数据

##  技术架构

### 技术栈

| 技术 | 版本 | 说明 |
|------|------|------|
| Spring Boot | 2.7.18 | 基础框架 |
| Spring Cloud | 2021.0.8 | 微服务框架 |
| Spring Cloud Alibaba | 2021.0.5.0 | 阿里巴巴微服务组件 |
| Nacos | 2.2.3 | 服务注册与配置中心 |
| Spring Cloud Gateway | - | API网关 |
| OpenFeign | - | 服务调用 |
| MyBatis Plus | 3.5.3.1 | ORM框架 |
| MySQL | 8.0 | 关系型数据库 |
| Redis | 7.0 | 缓存数据库 |
| RabbitMQ | 3.12 | 消息队列 |
| Elasticsearch | 7.17 | 搜索引擎（可选） |
| Docker | - | 容器化部署 |

### 系统架构图

```
┌─────────────┐
│   客户端     │
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────────────┐
│          API Gateway (8080)             │
│  ┌──────────┬──────────┬──────────┐    │
│  │  路由    │  鉴权    │  限流    │    │
│  └──────────┴──────────┴──────────┘    │
└────┬────────────┬────────────┬─────────┘
     │            │            │
     ▼            ▼            ▼
┌─────────┐  ┌─────────┐  ┌─────────┐
│  用户服务 │  │  商品服务 │  │  订单服务 │
│  (8081)  │  │  (8082)  │  │  (8083)  │
└────┬────┘  └────┬────┘  └────┬────┘
     │            │            │
     └────────────┴────────────┘
                  │
     ┌────────────┼────────────┐
     ▼            ▼            ▼
┌────────┐   ┌────────┐   ┌────────┐
│  MySQL │   │  Redis │   │RabbitMQ│
└────────┘   └────────┘   └────────┘
```

## 📁 项目结构

```
distributed-ecommerce/
├── ecommerce-common/          # 公共模块
│   ├── exception/            # 统一异常处理
│   ├── result/               # 统一响应结果
│   ├── utils/                # 工具类（JWT、Redis锁等）
│   └── constant/             # 常量定义
├── ecommerce-gateway/         # 网关服务 (8080)
│   ├── filter/               # 鉴权、限流过滤器
│   └── config/               # 网关配置
├── ecommerce-user/            # 用户服务 (8081)
│   ├── controller/           # 用户接口
│   ├── service/              # 业务逻辑
│   ├── mapper/               # 数据访问
│   └── entity/               # 实体类
├── ecommerce-product/         # 商品服务 (8082)
│   ├── controller/           # 商品接口
│   ├── service/              # 库存管理、缓存
│   └── mapper/               # 数据访问
├── ecommerce-order/           # 订单服务 (8083)
│   ├── controller/           # 订单接口
│   ├── service/              # 订单创建、支付
│   ├── feign/                # Feign远程调用
│   └── mq/                   # RabbitMQ消息处理
├── ecommerce-frontend/        # 前端项目 (3000) 🆕
│   ├── src/
│   │   ├── api/              # API接口封装
│   │   ├── views/            # 页面组件
│   │   ├── router/           # 路由配置
│   │   └── utils/            # 工具函数
│   ├── package.json          # npm配置
│   └── vite.config.js        # Vite配置
├── docker/                    # Docker配置
│   └── mysql/init/           # 数据库初始化脚本
├── docker-compose.yml         # Docker编排文件
└── pom.xml                    # Maven父项目
```

##  快速开始

### 环境要求

- JDK 11+
- Maven 3.6+
- Docker & Docker Compose

### 1. 启动基础服务

```bash
# 启动 MySQL、Redis、RabbitMQ、Elasticsearch、Nacos
docker-compose up -d

# 查看服务状态
docker-compose ps
```

### 2. 访问Nacos控制台

- 地址：http://localhost:8848/nacos
- 账号/密码：nacos/nacos

### 3. 编译项目

```bash
# 根目录执行
mvn clean package -DskipTests
```

### 4. 启动微服务

```bash
# 方式1：IDEA中直接运行各个 Application 类

# 方式2：命令行启动
cd ecommerce-user
mvn spring-boot:run

cd ecommerce-product
mvn spring-boot:run

cd ecommerce-order
mvn spring-boot:run

cd ecommerce-gateway
mvn spring-boot:run
```

### 5. 启动前端（可选）

```bash
cd ecommerce-frontend

# 安装依赖（首次运行）
npm install

# 启动开发访问方式

### 方式1：前端界面访问 🆕 推荐

访问：http://localhost:3000

**测试账号**：
- 用户名：`testuser`
- 密码：`123456`

**功能页面**：
-  登录/注册
-  商品列表（搜索、分页）
-  商品详情
-  创建订单
-  我的订单（查看、支付、取消）

### 方式2：Postman测试API

详见 [QUICKSTART.md](QUICKSTART.md) 中的完整测试流程。

**基础URL**

访问 http://localhost:8848/nacos，确认以下服务已注册：
-  ecommerce-user (8081)
-  ecommerce-product (8082)
-  ecommerce-order (8083)
-  ecommerce-gateway (8080)
-  ecommerce-frontend (3000) 🆕

##  API接口文档

详见 [QUICKSTART.md](QUICKSTART.md) 中的完整测试流程。

### 基础URL

所有API请求通过网关访问：`http://localhost:8080/api`

##  核心功能实现

### 1. 分布式锁防止超卖

**位置**：[ProductServiceImpl.java](ecommerce-product/src/main/java/com/ecommerce/product/service/impl/ProductServiceImpl.java)

```java
// 获取分布式锁
String lockValue = redisLockUtil.tryLockWithRetry(lockKey, 10, 3, 100);

// 扣减库存
int result = productMapper.deductStock(productId, quantity);
```

### 2. 接口幂等性设计

**位置**：[OrderServiceImpl.java](ecommerce-order/src/main/java/com/ecommerce/order/service/impl/OrderServiceImpl.java)

```java
// Token校验（只能使用一次）
Boolean deleted = redisTemplate.delete(tokenKey);
if (!Boolean.TRUE.equals(deleted)) {
    throw new BusinessException(ResultCode.DUPLICATE_REQUEST);
}
```

### 3. 订单超时自动取消

**位置**：[OrderMessageConsumer.java](ecommerce-order/src/main/java/com/ecommerce/order/mq/OrderMessageConsumer.java)

```java
// 30分钟后检查订单状态
if (order.getStatus() == 0) {
    orderService.cancelOrder(orderNo);
}
```

### 4. JWT认证

**位置**：[AuthFilter.java](ecommerce-gateway/src/main/java/com/ecommerce/gateway/filter/AuthFilter.java)

```java
// 验证Token
boolean valid = JwtUtil.validateToken(token);
```


## 📈 性能优化

-  Redis缓存热点商品（30分钟）
-  MyBatis Plus批量查询
-  数据库连接池（Druid）
-  异步消息处理（RabbitMQ）
-  网关层限流降级

## 🔧 待扩展功能

- [ ] Elasticsearch商品搜索
- [ ] Sentinel限流降级
- [ ] Skywalking链路追踪
- [ ] 购物车功能
- [ ] 优惠券系统
- [ ] 推荐系统


## 📄 相关文档

- 📘 [快速开始指南](QUICKSTART.md) - 5分钟快速启动项目
- 📙 [API接口文档](QUICKSTART.md#测试接口) - 完整API测试流程

## 🤝 常见问题

详见 [QUICKSTART.md](QUICKSTART.md#常见问题)



---

 **如果这个项目对你有帮助，请给个⭐支持一下！**
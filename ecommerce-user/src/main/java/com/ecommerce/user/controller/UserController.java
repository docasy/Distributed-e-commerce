...
public class UserController {
    ...
    @PutMapping("/profile")
    @ApiOperation("更新个人信息")
    public Result<Void> updateProfile(@RequestHeader("Authorization") String token, @RequestBody User updateData) {
        Long userId = JwtUtil.getUserId(token);
        User user = userService.getUserById(userId);
        if (user == null) {
            return Result.error("用户不存在");
        }
        if (updateData.getNickname() != null && !updateData.getNickname().isEmpty()) {
